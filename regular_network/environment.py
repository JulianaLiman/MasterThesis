...
class WMSEnvironment(Env):
    ...
    def step(self, action):
        ...
        if self.steps_left <= 0 or step_reward==0:
            done = True
        else:
            done = False
        ...
        if done:
            ...
            for i in range(len(self.designated_link)):
                ...
                if len(com_chan) != 0:
                    common_channel.append(com_chan)
                    working_link.append(self.designated_link[i])
                    working_distance.append(self.designated_distance[i])

            # check if there are more than 2 common channel
            multiple_channel = []
            multiple_channel_link = []
            for i in range(len(working_link)):
                if len(common_channel[i]) > 1:
                    multiple_channel_link.append(working_link[i])
                    multiple_channel.append(common_channel[i])
            ...
            if len(multiple_channel_link) != 0:
                ...
                for i in range(len(multiple_channel_link)):
                    ...
                    for j in range(len(a)):
                        neighbour_count = 0
                        for k in range(len(working_link)):
                            b = working_link[k]
                            if a != b:
                ...
                for i in range(len(multiple_channel_link)):
                    ...
                    x = working_link.index(multiple_channel_link[i])
                    common_channel[x] = chosen_common_channel
            ...
            for i in range(len(working_link)):
                fik_sum_1 = 0
                fik_sum_2 = 0
                if working_distance[i] < 0:
                    print('working distance [i]', working_distance[i])
                link_path_loss = self.path_loss_reff + 10 * self.path_loss_exponent * math.log(
                    (working_distance[i] / self.reference_distance), 10) + self.xg
                link_rss = (10 ** (-link_path_loss / 10)) * self.transmit_power
                a = working_link[i]
                for j in range(len(self.player)):
                    if isinstance(common_channel[i], np.int64) or isinstance(common_channel[i], np.int32) or isinstance(
                            common_channel[i], int):
                        com_chan = common_channel[i]
                    else:
                        com_chan = list(common_channel[i])
                        com_chan = com_chan[0]
                ...
                if sinr_1 > 1:
                    ...
                    if sinr_2 > 1:
                        sinr_list.append(working_link[i])
